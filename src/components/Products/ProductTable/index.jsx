...
import { setQuantityProduct, removeProduct } from '@/redux/Cart'
    ...
    const columns = [
        {
            ...
            render: (item) => {
                ...
                return (
                    <div className="info">
                        <Link to={`/product/${item?.attributes?.slug}`}><img className="thumbnail" src={imgProduct} alt={item?.attributes?.name} /></Link>
                        <div>
        ...
        },
        {
            ...
            render: item => (
                <div>
                    <span className="old-price">{convertTocurrency(item?.oldPrice)}</span> - <span className="price">{convertTocurrency(item?.price)}</span>                </div>
            )
        },
        {
            title: 'Số lượng',
